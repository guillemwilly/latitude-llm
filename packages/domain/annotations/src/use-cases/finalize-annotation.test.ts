...
import { finalizeAnnotationUseCase } from "./finalize-annotation.ts"
...
function createTestLayers(initialScore?: Score) {
  ...
  const ScoreRepositoryTest = Layer.succeed(ScoreRepository, {
    ...
    listBySource: () => Effect.succeed({ items: [], hasMore: false, limit: 50, offset: 0 } as ScoreListPage),
  ...
  const AITest = Layer.succeed(AI, {
    generateText: (input) =>
      Effect.succeed({
        text: `Enriched: ${input.prompt.match(/"(.+?)"/)?.[1] ?? ""}`,
        tokens: 15,
        duration: 50_000_000,
      }),
  })
...
describe("finalizeAnnotationUseCase", () => {
  it("finalizes a draft annotation: clears draftedAt, enriches feedback", async () => {
    const draft = buildDraftAnnotationScore()
    const { store, layer } = createTestLayers(draft)

    const result = await Effect.runPromise(
      finalizeAnnotationUseCase({ scoreId: scoreCuid }).pipe(Effect.provide(layer)),
    )
  ...
  it("is idempotent on already-finalized annotation", async () => {
    const finalized = {
      ...buildDraftAnnotationScore(),
      draftedAt: null,
    } as Score
    const { events, layer } = createTestLayers(finalized)
  ...
  it("emits ScoreImmutable for failed non-errored annotations without issue", async () => {
    ...
    await Effect.runPromise(finalizeAnnotationUseCase({ scoreId: scoreCuid }).pipe(Effect.provide(layer)))
  ...
  it("returns BadRequestError for non-existent score", async () => {
    ...
    const result = await Effect.runPromiseExit(
      finalizeAnnotationUseCase({ scoreId: missingId }).pipe(Effect.provide(layer)),
    )
