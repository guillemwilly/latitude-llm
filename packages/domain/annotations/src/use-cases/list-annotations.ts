import { baseListScoresInputSchema, type ScoreListPage, ScoreRepository, scoreDraftModeSchema } from "@domain/scores"
...
export const listProjectAnnotationsUseCase = (input: ListProjectAnnotationsInput) =>
  Effect.gen(function* () {
    ...
    return yield* scoreRepository.listBySource({
      projectId: parsed.projectId,
      source: "annotation",
      options: {
        limit: parsed.limit,
...
export const listTraceAnnotationsUseCase = (input: ListTraceAnnotationsInput) =>
  Effect.gen(function* () {
    ...
    }) as Effect.Effect<ScoreListPage, RepositoryError>
  })
